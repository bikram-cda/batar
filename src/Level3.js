...
export function Level3({setPNo, apiData}) {
  ...
  const backgroundImage = apiData ? apiData['7'].find((image) => image.key === 'Background.png') : null;
    const Banner = apiData?.['7'].find((image) => image.key === 'Banner.png');
    const ProgressBar = apiData?.['7'].find((image) => image.key === 'Process Bar.png');
