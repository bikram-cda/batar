...
export function Level2({setPNo, apiData}) {
  ...
  const handleButtonClick = () => {
    ...
    const backgroundImage = apiData ? apiData['5'].find((image) => image.key === 'Kiosk background.png') : null;
    const Banner = apiData?.['5'].find((image) => image.key === 'Banner.png');
    const ProgressBar = apiData?.['5'].find((image) => image.key === 'Process Bar.png');
